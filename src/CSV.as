package 
{
	...
	{
		public static function encode(data:Array,btwKey:String=","):String
		{
			if (data == null)
				return null;
			...
			for each (var line:Array in data)
			{
				...
				newData.push(newLine.join(btwKey))
			}
			return newData.join("\r\n");
			
		}
		public static function decode(text:String,btwKey:String=","):Array
		{
			if (text == null)
				return null;
			...
			if (line.length > 0)
				...
				//{
					//for (var j:int = 0; j < result[0].length; j++) 
					//{
						//tmpstr = result[0][j];
						//trace(tmpstr,StringTools.(tmpstr));
						//result[0][j] = StringTools.utf8to16(tmpstr);
					//}
				//}
